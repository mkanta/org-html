* Equations
Solving equations involves clearing denominators, expanding terms to remove
parentheses and combining the terms that contain the variable to solve for and 
those that don't. These two types of terms are then assembled at opposite sides
of the equality sign and the further treatment of the equation depends on its
kind. 

As an example, consider \[4(x-3)+2=15-5(x+6).\] Expanding the left hand side
of the equation leads to  
\[4\,x-12+2=15-5(x+6)\] and doing the same on the right results in
...
\[9\,x=-5.\] Finally, dividing both sides by $9$ gives the result
\[x=-{\frac{5}{9}}.\]
