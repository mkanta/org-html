...
and $g$ the acceleration due to gravity, about $9.81\frac{m}{s^2}$. 
This means
it traces out a parabola in a coordinate system plotting displacement $s$ vs. 
time $t$ looking like figure ([[fig:verticalThrow]]).
#+CAPTION: Vertical Component of a Thrown Object
#+NAME: fig:verticalThrow
 [[./throw.png]]
...
Nevertheless, it is still possible to define tangent lines in more general cases
such as the above, but a bit more care needs to be taken and a more thorough
analysis of what such a definition is supposed to accomplish.
** Ellipses and Circles
Ellipses are conic sections whose general form ([[generalConic]]) can be 
converted to the form
#+NAME: normalEllipse
\begin{equation}
\frac{\left(x-c_x\right)^2}{a^2}+\frac{\left(y-c_y\right)^2}{b^2}=1.
\end{equation}
The particular case where $a=b$ is the circle where the common value
of $a$ and $b$ is called $r$ because it represents the radius of the
circle. The normal form of the circle is usually written as
#+NAME: normalCircle
\begin{equation}
\left(x-c_x\right)^2+\left(y-c_y\right)^2=r^2,
\end{equation}
which is obtained from the normal form ([[normalEllipse]]) of the ellipse
by multiplying equation ([[normalEllipse]]) by the common value $r^2=a^2=b^2$.
** Hyperbolas
In the conic sections presented so far, the signs of the coefficients
of the quadratic terms of the normal forms, if both present, where the same,
ie they were either both positive or both negative. Hyperbolas, on the other
hand are characterized by opposite signs of these coefficients, with their
normal form given by
#+NAME: normalHyperbola
\begin{equation}
\frac{\left(x-c_x\right)^2}{a^2}-\frac{\left(y-c_y\right)^2}{b^2}=1
\end{equation}
or, exchanging $x$ and $y$ coordinates,
#+NAME: normalConjugateHyperbola
\begin{equation}
\frac{\left(y-c_y\right)^2}{a^2}-\frac{\left(x-c_x\right)^2}{b^2}=1
\end{equation}
with the latter often referred to as /conjugate/ hyperbola.
* TODO Functions                                                   :noexport:
